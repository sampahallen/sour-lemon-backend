...
const maxImageBytes = Number(process.env.JOURNAL_IMAGE_MAX_BYTES ?? 5 * 1024 * 1024)

if (!Number.isSafeInteger(maxImageBytes) || maxImageBytes <= 0) {
  throw new Error('JOURNAL_IMAGE_MAX_BYTES must be a positive integer')
}
...
export const validateJournalImageFile: RequestHandler = (request, _response, next) => {
  if (request.file && !hasExpectedSignature(request.file)) {
